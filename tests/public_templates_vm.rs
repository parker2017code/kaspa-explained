...
fn compile(v:&Value)->SilAbiArtifact{
 let source=std::fs::read_to_string(std::path::Path::new(&std::env::var("KE_PUBLIC_CONTRACT_DIR").unwrap()).join(string(&v["file"]))).unwrap();
 let args:Vec<ArtifactValue>=serde_json::from_value(v["constructorArgs"].clone()).unwrap();
...
#[test]
fn exact_public_template_transactions_and_adversarial_rejections(){
 let data:Value=serde_json::from_str(&std::fs::read_to_string(std::env::var("KE_PUBLIC_FIXTURES").unwrap()).unwrap()).unwrap();
 ...
 for fixture in fixtures{
  ...
  let artifact=compile(fixture);
  assert_eq!(bytecode(&artifact),bytes(string(&fixture["script"])),"patched state equals compiled contract: {name}");
  if !fixture["funding"].as_bool().unwrap(){
   let args:Vec<ArtifactValue>=serde_json::from_value(fixture["args"].clone()).unwrap();
   let mut encoded=encode_entry_sig_script(&artifact,string(&fixture["entry"]),&args).unwrap();encoded.extend(push_redeem_script(&bytecode(&artifact)));
   assert_eq!(tx.inputs[0].signature_script,encoded,"browser ABI: {name}");
   assert_eq!(entries[0].script_public_key,kaspa_txscript::pay_to_script_hash_script(&bytecode(&artifact)),"input template: {name}");
  }else{assert_eq!(tx.outputs[0].script_public_key,kaspa_txscript::pay_to_script_hash_script(&bytecode(&artifact)),"funding template: {name}");}
