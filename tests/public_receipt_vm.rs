// Executes the exact signed SDK transactions generated by server/backed-receipt.mjs --fixtures.
...
fn state(v:&Value)->ArtifactValue {BTreeMap::from([
 ("series".to_string(),ArtifactValue::Bytes(bytes(string(&v["series"])))),
 ("maxFee".to_string(),ArtifactValue::Int(v["maxFee"].as_i64().unwrap())),
 ("owner".to_string(),ArtifactValue::Bytes(bytes(string(&v["owner"])))),
 ("quantity".to_string(),ArtifactValue::Int(v["quantity"].as_i64().unwrap())),
 ("seriesId".to_string(),ArtifactValue::Bytes(bytes(string(&v["seriesId"]))))
]).into()}
fn compile(v:&Value)->SilAbiArtifact{
 let source=std::fs::read_to_string(std::path::Path::new(&std::env::var("KE_CONTRACT_DIR").unwrap()).join("backed-receipt.sil")).unwrap();
 let args=[ArtifactValue::Bytes(bytes(string(&v["series"]))),ArtifactValue::Int(v["maxFee"].as_i64().unwrap()),ArtifactValue::Bytes(bytes(string(&v["state"]["owner"]))),ArtifactValue::Int(v["state"]["quantity"].as_i64().unwrap())];
...
fn exact_sdk_transactions_and_adversarial_rejections(){
 let data:Value=serde_json::from_str(&std::fs::read_to_string(std::env::var("KE_BACKED_FIXTURES").unwrap()).unwrap()).unwrap();
 assert_eq!(data["network"],"testnet-10");assert_eq!(data["unfunded"],true);
 let fixtures=data["fixtures"].as_array().unwrap();assert!(fixtures.len()>=27);
 for fixture in fixtures{
  ...
  for (i,t) in fixture["receipts"].as_array().unwrap().iter().enumerate(){
   let a=compile(t);let signature=ArtifactValue::Bytes(bytes(string(&fixture["signatures"][i])));
   let args=if i==0{vec![ArtifactValue::Array(fixture["states"].as_array().unwrap().iter().map(state).collect()),ArtifactValue::Int(fixture["operation"].as_i64().unwrap()),ArtifactValue::Bytes(bytes(string(&fixture["sponsor"]))),signature]}else{vec![signature]};
  ...
  else{assert!(result.is_err(),"must reject {name}");if name=="reject-insufficient-committed-budget"{assert!(matches!(result,Err(TxScriptError::ExceededCommittedScriptUnits{..})),"{result:?}");}println!("{name}: {:?}",result.unwrap_err());}
 }
}
