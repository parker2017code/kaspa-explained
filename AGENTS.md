...
- **`PROSE_STANDARD.md` is the current prose authority (owner-authored, 30 July 2026).** It outranks the older word lists in `COPY_STYLE.md` wherever they disagree. Its measurable half runs in the gate as `check-prose.py`: ban-list hits, sentence-length variance, uniform-paragraph runs, rationed-device counts, and reading grade. The unmeasurable half is payload per sentence, which is the one that matters most and still needs a reader.
