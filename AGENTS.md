...
Follow the global working agreement at `/Users/parkerschmidt/.codex/AGENTS.md`.
This file adds project-specific guidance; the current user request and newer
global agreement govern wherever older local wording conflicts.

Use the global role split: Astra low owns discretionary goal interpretation,
product, architecture, and execution approach choices, scope, priorities, complete worker briefs,
evidence monitoring, and acceptance. Luna max performs substantive research,
implementation, fixes, execution/testing, and self-review under those briefs.
If evidence contradicts the brief or a consequential choice is unspecified,
Luna reports the evidence and options to Astra. Keep Astra's review focused and
do not duplicate broad research or test matrices.

## Read these two first, before anything else in this file

- **`PRINCIPLES.md`**: how to work. The owner's governing principles, stated
  repeatedly and unprompted, plus the full reasoning behind each. **These guide
  project quality within the current user request and global working agreement;
  they do not override the current user request, global agreement, an Astra brief,
  or the authorization and acceptance boundaries.** If a brief appears to conflict
  with them, report the evidence to Astra rather than silently reinterpreting the
  brief.
...
**The current user request and global working agreement are canonical. This file is project context.**

Every rule in this repo was written by a model, and models improve. The word lists, the voice notes, the checklists: all of them are one earlier model's best attempt at describing something it could only partly see. If a project rule is wrong, outdated, or too crude, report that evidence to Astra and correct it when the brief authorizes the change. Do not silently override a current user request or global role boundary.

**Routine technical execution within an explicit brief does not need a separate approval.** Changes to product, architecture, scope, priorities, acceptance, security, permissions, dependencies, external behavior, or cross-owner resources must be escalated to Astra with evidence and options before action. This preserves useful project judgment while keeping the global role and authorization boundaries intact.

Ask first for genuinely destructive or hard-to-reverse actions, and escalate the consequential choices listed above under the global working agreement. Normal content and design execution is routine only when it stays within the explicit brief.
...
- This repo is the public static Kaspa Explained site, deployed as Cloudflare Worker assets.
- These instructions apply to all agent work in this repository unless a deeper `AGENTS.md` adds narrower project context; the current user request and global role, authorization, and acceptance rules still govern.
...
- Do not change lockfiles, package managers, auth, deployment, domain, analytics, or Cloudflare settings without calling it out before the edit.
...
- Before committing, run the cheap gate for the touched surface. Before pushing public content, run the publish gate below and verify the live page after Cloudflare deploys.
...
- Public AI and source rules live in `llms.txt` and `sources.html`; keep them aligned with `CLAIMS.yml` and `CONTENT_BRIEF.md` when claim categories change. (`ai-guidance.html` carried these until it was retired to a redirect stub into `/status`.)
...
- For broad cleanup work, assign audits and research to Luna workers when available, with clear file ownership. Astra decides scope and approach and performs focused acceptance; do not repeat broad audits or test matrices in the coordinator.
- Avoid price predictions, exchange rumors, or investment advice.

## Startup Verification

- **Refresh the trackers every session (added 2026-07-29, corrected 2026-08-07, corrected 2026-08-22).** Two surfaces carry a dated baseline that is meant to move: the KIP and KCC tables on `kips.html`, and the `argent`, `vprogs`, and `kcc_conventions` entries in `CLAIMS.yml`. The repository pulse table this rule used to name on `kaspa-vprogs-explained.html` is gone: that page is now a redirect stub into `/build-on-kaspa#routes`, and no pulse table survived the merge, so `CLAIMS.yml`'s `vprogs` entry is the only place the vProgs repo state is recorded. `argent-explained.html` is now its own live page (created 2026-08-22, not a redirect) and carries its own repository-pulse table live from GitHub; it is no longer folded into the retired `toccata-explained.html`, which is itself now a stub into `/what-is-kaspa#covenants`. Each reads live from GitHub in the browser, so a visitor sees current data even when the baseline is old, but the baseline is what shows if GitHub rate-limits the request and it is what the claim registry is judged against. At session start, re-read `kaspanet/kips` README, `kaspanet/kccs` open pull requests, `kaspanet/vprogs`, and the `argent-lang` org, then update the baselines, the stamped dates, and `last_checked` in `CLAIMS.yml`. Bump `recheck_after` only after an actual source check, never to silence the gate.
- **`PROSE_STANDARD.md` v2.0 (1 Aug 2026) governs the page as an object, not just its sentences.** Reading grade on shared copy is capped at 9 and enforced by `scripts/check-reading-grade.py` inside the publish gate; mark a specialist-audience block `data-audience="specialist"` to have it measured separately rather than failed. The other v2.0 rules are not machine-checkable and still bind: never lead with the method, state the central claim twice at most, keep the word count falling, cap nav at five, group long card sets into six families with one line each, vary card openings, never end a card on its limit, ship numbers or omit the section, and check what sits above the fold at 390px. Version 1.0 below still applies to sentences.
- **`PROSE_STANDARD.md` v1.0 (owner-authored, 30 July 2026).** It outranks the older word lists in `COPY_STYLE.md` wherever they disagree. Its measurable half runs in the gate as `check-prose.py`: ban-list hits, sentence-length variance, uniform-paragraph runs, rationed-device counts, and reading grade. The unmeasurable half is payload per sentence, which is the one that matters most and still needs a reader.
...
- Check primary or near-primary sources first: Kaspa Research, KIPs, `kaspanet/rusty-kaspa`, `kaspanet/vprogs`, release notes, core technical contributor posts, and durable transcripts. Do NOT use kaspa.org as a source (owner instruction, 2026-08-22). It is unreliable on exactly the claims this site exists to get right. Confirmed twice: on 2026-07-11 and again on 2026-08-22 its `/developments` and `/build` pages still described Toccata as "currently live on TN12 ahead of mainnet activation", roughly eight weeks after Toccata activated on mainnet at DAA score 474,165,565. A docs page that is two months stale on the single biggest protocol event is not orientation, it is a source of errors. Use the repos, the release tags, the KIP and KCC files, kas-smiths.org, the Telegram Core R&D group, and core contributor posts instead. If kaspa.org is the only place a claim appears, that is a reason to doubt the claim.
  - **The kaspa.org ban is scoped to the marketing site, not the whole domain (clarified 2026-08-22, after an agent left `wiki.kaspa.org` citations in place under the ambiguity above).** Three subdomains, three different rulings:
    - `kaspa.org` marketing and narrative pages (`/`, `/lore`, `/build`, `/developments`, `/hodl`, `/assets`, and their `/es` equivalents) are NOT valid sources for any claim, for the reason above: eight weeks stale on Toccata activation as of 2026-08-22, with two GitHub issues open upstream about it.
    - `wiki.kaspa.org` data pages ARE acceptable as sources. It is a separate community wiki, different in kind and in staleness profile from the marketing site; `CLAIMS.yml` already cites `wiki.kaspa.org/en/prehistory` and `/en/tokenomics`.
    - `docs.kaspa.org` is acceptable. It was independently confirmed correct on Toccata activation on 2026-08-22, the same day the marketing pages were confirmed wrong.
    - Do not extend the ban past the marketing site by inference, and do not read this carve-out as reopening the marketing site.
...
- After pushing, confirm the Cloudflare deployment completed, then fetch the live page HTML for the exact changed copy.
