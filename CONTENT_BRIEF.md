...
Working estimate: site cleanup/tidying is roughly `90-92%`. Public claim discipline is stronger than repo maintainability; the next gains come from generating nav/search inventory from `site-manifest.json` and continued README/source-map reduction.
...
4. Keep page, support-file, and sitemap-extra inventory in `site-manifest.json`; sitemap is generated from the manifest and page metadata. Move nav, source, and search generation toward the manifest when manual sync becomes the bigger risk.
...
> Kaspa asks whether Proof-of-Work can keep its security model while feeling closer to real time: fast mined ordering today, receipt rails around the live network, and future apps that prove their own rules.
...
Based rollups should be a future execution lane, not a prerequisite for current Toccata examples. Direct L1 covenant examples such as vaults, escrow, and assurance can be explained without an L2. If core or core-adjacent work offers a based rollup path that is explicitly migratable toward vProgs, it is worth tracking for richer app state, DeFi prototypes, and rollup-side UX. Keep ecosystem L2 projects out of the site's assumptions unless a page is explicitly about ecosystem projects.
