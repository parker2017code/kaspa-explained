...
Working estimate: site cleanup/tidying is roughly `89-91%`. Public claim discipline is stronger than repo maintainability; the next gains come from generating nav/sitemap/search inventory from `site-manifest.json` and continued README/source-map reduction.
...
4. Keep page, support-file, and sitemap-extra inventory in `site-manifest.json`; move nav, source, sitemap, and search generation toward the manifest when manual sync becomes the bigger risk.
