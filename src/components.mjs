...
export function inspector() {
  ...
  return `<section class="inspector" data-inspector><form data-lookup-form><label for="transaction-id">Inspect a mainnet transaction</label><div class="lookup-line"><input id="transaction-id" name="transaction" type="text" maxlength="64" pattern="[a-fA-F0-9]{64}" spellcheck="false" autocomplete="off" placeholder="Paste a transaction ID" required><button class="primary-button" type="submit">Look up</button></div><p class="small">Looking up sends the ID to api.kaspa.org through this local service. No wallet connection. Never enter a recovery phrase.</p></form><p data-lookup-message role="status"></p><div data-lookup-result></div></section>`;
}
...
export function tradeoffComparison(){
  ...
  return `<div class="comparison" data-lesson><nav class="lesson-steps" aria-label="Compare design tradeoffs">${items.map(([name],i)=>`<button data-lesson-step="${i}" aria-pressed="${i===0}">${name}</button>`).join('')}</nav>${items.map(([name,benefit,cost,visual],i)=>`<div class="comparison-panel" data-lesson-panel="${i}"${i?' hidden':''}><div class="comparison-object">${visual}<span>${name}</span></div><div><p class="eyebrow">What this enables</p><h3>${benefit}</h3></div><div><p class="eyebrow">What it requires</p><p>${cost}</p></div></div>`).join('')}</div>`;
}
