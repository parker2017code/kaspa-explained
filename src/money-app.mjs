import {mountMoneyDiagrams,renderMoneyDiagram} from './mechanism-diagrams.mjs';
...
next.addEventListener('click',advanceTour);again.addEventListener('click',()=>{tourIndex=-1;next.disabled=false;advanceTour();next.focus();});

root.dataset.guided='true';const explore=document.createElement('button');explore.className='quiet-button walkthrough-explore';explore.textContent='Explore controls';explore.setAttribute('aria-expanded','false');walkthrough.append(explore);explore.addEventListener('click',()=>{const expanded=root.dataset.guided==='true';root.dataset.guided=String(!expanded);explore.setAttribute('aria-expanded',String(expanded));explore.textContent=expanded?'Hide controls':'Explore controls';});
