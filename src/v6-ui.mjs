...
function renderChapterDirectory(view) {
  ...
  return `<nav class="v6-directory" aria-label="Learning chapters"><div class="v6-directory-heading"><div><p class="v6-eyebrow">Sprout Harbor learning tour</p><strong>${v6CompletedCount(view.progress)} of 6 chapters</strong><span>About ${v6RemainingMinutes(view.progress)} minutes remaining</span></div><p class="v6-directory-note">Follow one consequence at a time. You can revisit a chapter after the tour.</p></div><ol>${V6_CHAPTERS.map(lesson => {
...
function renderInfo(view) {
  ...
  const error = view.error ? `<div class="v6-error" role="alert"><strong>Nothing moved</strong><p>${esc(view.error?.message || view.error)}</p></div>` : '';
  const chapterAction = view.step === 'complete' && view.chapter === V6_CHAPTERS.length - 1
    ...
    : view.step === 'complete' || (complete && view.step === 'outcome')
      ...
      : '';
  return `<div class="v6-info-scroll"><div class="v6-info-inner"><header class="v6-info-heading"><div><p class="v6-eyebrow">Chapter ${lesson.index + 1} · ${esc(lesson.place)}</p><h1>${esc(view.title || lesson.title)}</h1></div><button type="button" class="v6-dock-button" data-v6-dock aria-pressed="true">${renderIcon('pin')}<span>Dock guide</span></button></header>${renderCausalRail(view)}<section class="v6-causal-grid" aria-label="What is at stake"><article class="v6-causal-card is-problem"><p class="v6-eyebrow">Problem</p><h2>${esc(view.problem || lesson.problem)}</h2></article><article class="v6-causal-card is-rule"><p class="v6-eyebrow">Rule</p><h2>${esc(view.rule || lesson.rule)}</h2></article></section><section class="v6-action-card" data-v6-action-card aria-labelledby="v6-action-title"><div class="v6-card-heading"><div><p class="v6-eyebrow">${view.step === 'outcome' || view.step === 'complete' ? 'What changed' : view.step === 'pending' ? 'Observe' : 'Authorize'}</p><h2 id="v6-action-title">${esc(copy?.title || 'Choose the next step')}</h2></div>${connected ? `<span class="v6-demo-badge">Demo accounts</span>` : ''}</div><p>${esc(copy?.body || '')}</p></section>${operation}${result}<section class="v6-support-card" data-v6-evidence aria-labelledby="v6-evidence-title"><div class="v6-card-heading"><div><p class="v6-eyebrow">How to read this</p><h2 id="v6-evidence-title">Technology in context</h2></div><span class="v6-boundary-mark">Testnet-10</span></div>${renderTech(view.evidence?.technology || lesson.technology)}${Array.isArray(view.evidence?.checked) && view.evidence.checked.length ? `<div class="v6-checked"><strong>Checked for this result</strong><ul>${view.evidence.checked.map(item => `<li>${esc(item)}</li>`).join('')}</ul></div>` : ''}<p class="v6-boundary"><strong>Boundary</strong> ${esc(view.evidence?.boundary || lesson.boundary)}</p></section><details class="v6-history-card"><summary>Session history <span>${Array.isArray(view.history) ? view.history.length : 0}</span></summary>${renderHistory(view.history)}</details></div></div><div class="v6-action-footer" aria-label="Current chapter action">${error}<div class="v6-action-row">${chapterAction || `${actionButton(actionType, actionLabel, {chapter: lesson.index, step: view.step}, disabled, 'v6-button-primary')}${secondary && connected ? actionButton(secondary.type || 'try_attack', secondary.label, secondary.payload || {chapter: lesson.index, step: view.step}, disabled, 'v6-button-secondary') : ''}`}</div>${view.busy ? '<p class="v6-busy" role="status" aria-live="polite">Working with the saved session…</p>' : ''}</div>`;
}
...
function mountMarkup() {
  return `<header class="v6-header"><a class="v6-brand" href="/" aria-label="Kaspa Explained home">${renderIcon('leaf')}<span><strong>Sprout Harbor</strong><small>A Kaspa learning experience</small></span></a><div class="v6-header-status"><span class="v6-network-pill"><i data-v6-network-dot></i><span data-v6-network-status>Testnet-10 · ready for the guide</span></span><span class="v6-wallet-pill" data-v6-wallet-display>Demo accounts</span><span class="v6-time-pill" data-v6-time>About 12–15 min</span></div></header><div data-v6-directory-host></div><main class="v6-main"><section class="v6-scene-pane" aria-label="Interactive harbor scene"><div class="v6-scene-toolbar"><span data-v6-scene-place>Exchange</span><span data-v6-scene-hint>Scene motion follows the saved result</span></div><div class="v6-scene" data-v6-scene data-v6-kind="exchange"><div class="v6-scene-placeholder" data-v6-scene-placeholder><span>${renderIcon('harbor')}</span><strong>Sprout Harbor</strong><small>The interactive scene will appear here.</small></div></div><div data-v6-scene-evidence-host></div><div data-v6-scene-caption></div></section><button type="button" class="v6-resizer" data-v6-resizer role="separator" aria-label="Resize scene and explanation pane" aria-orientation="vertical" aria-valuemin="300" aria-valuemax="520" aria-valuenow="360">${renderIcon('grip')}</button><aside class="v6-info-pane" data-v6-info-pane aria-label="Chapter explanation"><div data-v6-info></div></aside></main><footer class="v6-footer"><span>Demo accounts · Kaspa Testnet-10 · experimental and unaudited</span><span>Use the guide to see what the transaction changes.</span></footer>`;
}
