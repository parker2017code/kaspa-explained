...
for(const inspector of all('[data-inspector]')) one('form',inspector).addEventListener('submit',async event=>{
  ...
  try{
    const response=await fetch(`/api/transaction/${id}`,{signal:AbortSignal.timeout(12000)}),record=await response.json();if(!response.ok)throw new Error(record.error||'Lookup unavailable.');
