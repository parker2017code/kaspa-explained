...
export function networkDiagram(s,{interactive=false,selected=null,id='network'}={}){
  ...
  const svg=compact=>{
    ...
    return `<svg class="network-drawing ledger-drawing ${compact?'drawing-small':'drawing-wide'}" viewBox="0 0 ${compact?`360 ${s.foundC?350:170}`:'720 380'}" role="group" aria-label="${s.parallel?'Parallel block references':'Sequential block references'}"><defs><marker id="${marker}" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto"><path d="M1 1 L8 5 L1 9" fill="none" stroke="currentColor" stroke-width="1.4"/></marker></defs>${edges}${blocks}</svg>`;
  };
