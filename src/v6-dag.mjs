...
export function mountV6Dag(root){
  ...
  function paint(now=performance.now()){
    ...
    if(HASH.test(id||'')){
      let end=positions.get(transaction.acceptingBlock);if(accepted&&(!end||end.x<20||end.x>740)){end={x:650,y:60};nodes+=`<g class="is-accepting pinned" transform="translate(${end.x},${end.y})"><title>Observed accepting block ${esc(transaction.acceptingBlock)}</title><rect x="-14" y="-12" width="28" height="24" rx="5"/><text y="29" text-anchor="middle">${esc(transaction.acceptingBlock.slice(0,8))}</text></g>`;}
    ...
    const stale=network.status!=='live'||Date.now()-(network.lastEventAt||0)>10000;status.textContent=network.status==='idle'?'Not connected':network.status==='paused'?'Live updates paused':stale?(network.status==='disconnected'?'Reconnecting':network.status==='stale'?'Waiting for fresh blocks':'Connecting'):'Live';status.dataset.live=String(!stale);
