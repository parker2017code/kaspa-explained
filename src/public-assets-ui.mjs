...
export function createPublicAssetsUI(panel,host){
 ...
 function display(){
  ...
  const descriptions={token:['Learning token','Create and move a token.','A custom token with a fixed issuance cap. Its units are separate from the test coins carried by each holding.'],receipt:['Backed receipt','Hold a claim on test coins.','The contract holds the full amount of test coins claimed by each receipt. Move or split the claim, then redeem it to the holder.'],payment:['Test payment','Send a small amount.','Move Testnet-10 coins between your example accounts. Review the destination, amount, and fee first.']};
 ...
 for(const name of ['recipient','recipient-two','amount','amount-two','split'])q(name).addEventListener('input',()=>{review=null;q('review').hidden=true;if(name==='split')display();});
