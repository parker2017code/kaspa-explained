...
Prepared September 6, 2026. Publication is pending; these notes describe the release candidate.
...
The prepared Actions workflow publishes only `dist-v1`. Confirm the repository's Pages source configuration before replacing the previous root-based site, run the final combined release gate, publish the reviewed source, and verify the live custom domain, routes, assets, historical links, and PDFs after deployment. No local check constitutes evidence that deployment has happened.
