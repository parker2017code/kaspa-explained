...
4. Take the successful job summary's immutable `ghcr.io/parker2017code/kaspa-explained-v6@sha256:…` source reference. The full-commit tag is convenient for lookup; the digest is the build identity. Rebuilding the same commit can change its digest when base images or system packages change.
