...
try{
 ...
 try{await page.goto(base+'/what-is-kaspa#parallel-blocks');await page.waitForFunction(()=>document.querySelector('[data-time]')?.value==='1200');assert.equal(await page.locator('[data-network-replay]').textContent(),'Watch it happen');await page.waitForTimeout(900);assert.equal(await page.locator('[data-time]').inputValue(),'1200');results.push({engine:'chromium',autoplay:'completed once and stopped'});
  ...
  await boundary.goto(base+'/what-is-kaspa#parallel-blocks');await boundary.waitForFunction(()=>document.querySelector('[data-network-replay]')?.textContent==='Pause',null,{polling:20});
