...
import {compileToken,buildTokenGenesis,buildTokenMove,buildTokenExchange,signTokenPlan,tokenPlanMass,preflightTokenPlan,TOKEN_NETWORK} from '../server/token-lab.mjs';
...
await mkdir(resolve(root,'.cache/token-fixtures'),{recursive:true});
...
const genesis=await signed('genesis',buildTokenGenesis(sdk,{fundingUtxos:[funding(10,31000000n)],token:initial,cellAmount:30000000n,fee:1000000n,changeAddress:addresses[0]}));
const mintFactory=()=>buildTokenMove(sdk,{tokenInputs:[{utxo:from(genesis,0),token:initial}],successors:[{token:minter,amount:14500000n},{token:holder,amount:14500000n}],operation:1,fee:1000000n});
const minted=await signed('mint',mintFactory());
const transferFactory=()=>buildTokenMove(sdk,{tokenInputs:[{utxo:from(minted,1),token:holder}],successors:[{token:recipient,amount:13500000n}],operation:0,fee:1000000n});
await signed('transfer',transferFactory());
const split=await signed('split',buildTokenMove(sdk,{tokenInputs:[{utxo:from(minted,1),token:holder}],successors:[{token:splitA,amount:16750000n},{token:splitB,amount:16750000n}],operation:0,fundingUtxos:[funding(12,20000000n)],fee:1000000n}));
const mergeFactory=()=>buildTokenMove(sdk,{tokenInputs:[{utxo:from(split,0),token:splitA},{utxo:from(split,1),token:splitB}],successors:[{token:recipient,amount:32500000n}],operation:0,fee:1000000n});
await signed('merge',mergeFactory());
const burnFactory=()=>buildTokenMove(sdk,{tokenInputs:[{utxo:from(minted,0),token:minter},{utxo:from(minted,1),token:holder}],successors:[{token:minter,amount:28000000n}],operation:2,fee:1000000n});
await signed('joint-burn',burnFactory());
const exchangeFactory=()=>buildTokenExchange(sdk,{sellerToken:{utxo:from(minted,1),token:holder},buyerFundingUtxos:[funding(11,20000000n,2)],buyerToken:recipient,price:8000000n,sellerAddress:addresses[1],buyerChangeAddress:addresses[2],fee:1000000n});
...
const template=transferFactory();template.transaction.outputs[0].scriptPublicKey=sdk.payToScriptHashScript(alteredTemplate.script);await signed('reject-output-cap-template',template,{valid:false});
const issuerTemplate=await compileToken({issuer:owners[3],cap:1000,state:recipient.state});
const issuer=transferFactory();issuer.transaction.outputs[0].scriptPublicKey=sdk.payToScriptHashScript(issuerTemplate.script);await signed('reject-output-issuer-template',issuer,{valid:false});
...
const duplicated=mintFactory();duplicated.states=[minter.state,{owner:owners[0],quantity:100,isMinter:true}];duplicated.transaction.outputs[1].scriptPublicKey=sdk.payToScriptHashScript((await token(0,100,true)).script);await signed('reject-second-minter',duplicated,{valid:false});
...
const exhausted=await signed('exhaust-allowance',buildTokenMove(sdk,{tokenInputs:[{utxo:from(minted,0),token:minter}],successors:[{token:zero,amount:16750000n},{token:lastIssued,amount:16750000n}],operation:1,fundingUtxos:[funding(13,20000000n)],fee:1000000n}));
...
await writeFile(resolve(root,'.cache/token-fixtures/transactions.json'),JSON.stringify({network:TOKEN_NETWORK,unfunded:true,fixtures},null,2));
...
if(process.argv.includes('--check-vm')){
 ...
 await copyFile(resolve(root,'tests/token_vm.rs'),resolve(upstream,'silverscript-lang/tests/kaspa_explained_token.rs'));
 execFileSync('cargo',['test','-p','silverscript-lang','--test','kaspa_explained_token','--locked','--','--nocapture'],{cwd:upstream,stdio:'inherit',env:{...process.env,KE_CONTRACT_DIR:resolve(root,'contracts'),KE_TOKEN_FIXTURES:resolve(root,'.cache/token-fixtures/transactions.json'),CARGO_NET_GIT_FETCH_WITH_CLI:'true'}});
}
