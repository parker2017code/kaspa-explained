...
- The dated baselines in the live trackers: `kips.html` (both tables) and
  ...
  table, so the vProgs repo state lives only in `CLAIMS.yml`'s `vprogs` entry,
...
Pushed and deployed are different states. The static asset tree is deployed to
Cloudflare with the repository's deploy workflow or the documented Wrangler
command. A push alone is not evidence that the Worker has updated.
