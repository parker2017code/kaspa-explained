...
Silverscript v1.0.0 (stable, published September 9 at 16:48:31 UTC, GitHub release API).
Argent remains not release-ready per its current README; its release/tag feeds are empty.
vProgs remains early development/prototype, with no tags. KIP-2 remains Proposed;
KIP-16/17/20/21 remain Active; KCC-1/2/20 remain Draft. Primary files were fetched today.
The current programmability overview and Sutton's based-apps gist still separate prototype
architecture and proof verification from production app evidence and external trust anchors.
Kaspa API halving returned 403, so network measurements retain their August observation dates;
the emission paragraph states score thresholds instead of mislabeling the August subsidy current.
Only these statuses were refreshed; other historical records below retain their original scope.
...
Static HTML served from a Cloudflare Worker. The checked-in asset builder keeps
the deployment tree limited to public files, with no application build system.
...
`main` deploys the static `dist/` asset tree to Cloudflare, custom domain
`kaspaexplained.com` via `CNAME`.

```bash
bash scripts/check-site.sh
python3 scripts/build-static-dist.py
