...
10. **NEW ON THE PAGE: a Pareto front, "What you cannot get cheaper."** Score against cost per task on a log axis, rebuilt from the live weights on every render, so moving a dial moves the line. Drawn as a staircase, never a diagonal: between two front points the only thing buyable is the cheaper one, and a diagonal would draw models that do not exist. Every scored model is a dot, always. Filled dot means on the line and measured on everything asked; hollow ring means on the line but not fully measured, and it carries a star; small gray dot means beaten outright. A legend says all three, and the caption states the total on the chart. The front is labeled by default; a "Name every model" control labels the beaten ones too, off by default because 22 names is more text than the frame holds comfortably. Every dot names itself on hover or keyboard focus regardless. Labels are placed against every box already placed, and a label with no free slot is dropped and counted in the caption rather than stacked on another.
